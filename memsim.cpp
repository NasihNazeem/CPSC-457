...
struct Simulator
{
    ...
    void allocate(int tag, int size)
    {
        ...
        if (size > partitions->size || partitions == all_blocks.end())
        {
            ...
            else
            {
                ...
                all_blocks.emplace_back(-1, blocks_requested, (all_blocks.back().addr + all_blocks.back().size));
        ...
        else if (size < partitions->size)
        {
            ...
            {
                if (std::next(partitions)->tag == -1)
                {                                                    // check if free block
                    partitions->size += std::next(partitions)->size; // add sizes
                    free_blocks.erase(std::next(partitions));
                    all_blocks.erase(std::next(partitions)); // erase from both set and list after merge and data update
                }
            }

            tagged_blocks[tag].push_back(std::prev(partitions)); // tag newly created partition to a vector
    ...
    void check_consistency_internal()
    {
        ...
        for (p = all_blocks.begin(); p != all_blocks.end(); ++p)
        {
            ...
            { // otherwise, checks if address is correct
